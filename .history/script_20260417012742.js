...
function getHonestyConsequenceImpact(text, level) {
    ...
    if (/минус\s*40\s*%\s*кристалл/.test(source)) {
        crystals = '-40%';
    ...
    if (/минус\s*\d+\s*%\s*xp|минус\s*xp|обнул.*xp|обнул.*опыт/.test(source)) {
        xp = 'штраф по условию';
    }

    if (level === 'light' && crystals === 'без потерь') {
        hp = 'до 0 (мягкий протокол)';
    }

    if (level === 'medium' && crystals === 'без потерь') {
...
function updateUI() {
    ...
    if (spinBtn) {
        ...
        if (spinHint) {
            if (blockReason) {
                spinHint.innerText = blockReason.text;
                spinHint.classList.add('warning');
            } else {
                spinHint.innerText = `Цена прокрутки ${BONUS_WHEEL_COST} 💎. Сегодня осталось ${Math.max(0, BONUS_SPINS_PER_DAY_LIMIT - Number(state.bonusWheel.spinsToday || 0))} прокруток.`;
                spinHint.classList.remove('warning');
            }
        }
    }
    ...
    const isLocked = isHonestyLockActive();
    if (navQuests) navQuests.classList.toggle('locked', isLocked);
    if (navStore) navStore.classList.toggle('locked', isLocked);
...
function renderHonestyProtocol() {
    ...
    listEl.innerHTML = consequences.map((text, index) => {
        ...
        return `
            <button class="honesty-consequence-item level-${level} ${active}" style="--reveal-delay:${delayMs}ms" onclick="selectHonestyConsequence(${index})">
                ...
                <span class="honesty-impact-row">
                    <span class="honesty-impact-chip hp">HP: ${impact.hp}</span>
                    <span class="honesty-impact-chip xp">XP: ${impact.xp}</span>
                    <span class="honesty-impact-chip crystals">💎: ${impact.crystals}</span>
                </span>
            </button>
        `;
    }).join('');
    ...
    if (!sorted.length) {
        container.innerHTML = `
            <div class="admin-empty">
                ...
                <p>После запуска протокола здесь появится карточка для подтверждения: сделал или не сделал.</p>
            </div>
        `;
        return;
    }
...
function drawWheel(canvasId, items, color1, color2) {
    ...
    for (let i = 0; i < slices; i++) {
        ...
        const fireBase = 'rgba(255, 98, 38, 0.95)';
        const waterBase = 'rgba(44, 196, 255, 0.92)';
        const base = isBonusWheel ? (i % 2 === 0 ? fireBase : waterBase) : (i % 2 === 0 ? color1 : color2);
    ...
    // Центральная ступица
    ctx.beginPath();
    const core = ctx.createRadialGradient(center, center, 2, center, center, radius * 0.22);
    core.addColorStop(0, 'rgba(168, 226, 255, 0.95)');
    core.addColorStop(0.4, 'rgba(38, 118, 188, 0.92)');
    core.addColorStop(1, '#08111f');
    ctx.fillStyle = core;
    ctx.arc(center, center, radius * 0.16, 0, Math.PI * 2);
    ctx.fill();
    ctx.lineWidth = 2;
    ctx.strokeStyle = 'rgba(149, 197, 255, 0.45)';
    ctx.stroke();

    ctx.fillStyle = 'rgba(228, 245, 255, 0.95)';
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
...
function checkPIN() {
    ...
    if (isMasterPinValid(enteredPin, { settings: { masterPin: state.settings.masterPin } })) {
        closeAdmin();
        switchTab('tab-admin', document.querySelector('.nav-admin'));
        return;
    }
