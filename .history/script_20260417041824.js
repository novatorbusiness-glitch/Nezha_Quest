...
function renderQuests() {
    ...
    const statusKey = QUEST_DATA
        ...
        .join('|');
    const nextRenderKey = `${isNight ? 1 : 0}|${statusKey}`;
...
function renderShop() {
    ...
    const nextRenderKey = `${state.crystals}|${state.isSpinning ? 1 : 0}|${isTxLocked() ? 1 : 0}`;
    if (container.dataset.renderKey === nextRenderKey) {
        return;
    }
    container.dataset.renderKey = nextRenderKey;
...
function renderHonestyProtocol() {
    ...
    const honestyKey = `${level}|${selected}|${pendingRequest ? pendingRequest.id : ''}|${latestRequest ? `${latestRequest.id}:${latestRequest.status}` : ''}|${state.honesty.requests.length}`;
