...
function renderHonestyProtocol() {
    ...
    listEl.innerHTML = consequences.map((text, index) => {
        ...
        return `<button class="honesty-consequence-item ${active}" onclick="selectHonestyConsequence(${index})">${text}</button>`;
    }).join('');
...
function triggerPenaltyProtocol(category, fixedPenaltyText = '') {
    if (!PENALTIES[category]) return;

    if (category === 'light' && state.shields > 0) {
        state.shields -= 1;
    ...
    trackEvent('penalty_triggered', { category, mode: state.penaltyMode, hpDelta: -100 });
