export function renderAdminScreen({
  ...
  if (!pendingQuests.length && !rejectedQuests.length && !pendingRewards.length && !pendingHonestyRequests.length && !approvedHonestyRequests.length) {
    return `
  ...
  const honestyApplyMarkup = approvedHonestyRequests
    .map((request) => {
      const levelLabel = levelLabelMap[request.level] || "Легкий";
      const penaltyHint = request.level === "hard"
        ? "Списание: обнулить 💎 и XP"
        : (request.level === "medium" ? "Списание: -30% 💎 и -20% XP" : "Списание: -10% 💎");

      return `
        <div class="quest-card approved admin-request-card admin-request-honesty-apply">
          <div class="quest-info">
            <h4 class="admin-request-headline"><span class="admin-request-glyph" aria-hidden="true"></span>${levelLabel} протокол подтвержден</h4>
            <span class="quest-reward">Последствие: ${request.consequence}</span>
            <span class="quest-reward">${penaltyHint}</span>
          </div>

          <div class="admin-actions">
            <button class="btn-action btn-confirm btn-master-approve" onclick="applyHonestyResourcePenalty('${request.id}')">ПРИМЕНИТЬ СПИСАНИЕ</button>
          </div>
        </div>
      `;
    })
  ...
  const blocks = [
    ...
    {
      title: "Подтверждено: Применить списание",
      count: approvedHonestyRequests.length,
      className: "honesty",
      markup: honestyApplyMarkup
    },
    {
